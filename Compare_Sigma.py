...
graph.subplot(234)
result = wiener(blurred_noisy, PSF, 0.01 + 1e-3)  # 对添加噪声的图像进行维纳滤波
graph.title("wiener deblurred(k=0.01)")
...
graph.subplot(232)
graph.title("motion & noisy blurred & sigma=0.1")
graph.gray()
graph.imshow(blurred_noisy)  # 显示添加噪声且运动模糊的图像
...
blurred_noisy = blurred + 1 * blurred.std() * \
                np.random.standard_normal(blurred.shape)  # 添加噪声,standard_normal产生随机的函数

graph.subplot(233)
graph.title("motion & noisy blurred & sigma=1")
graph.gray()
graph.imshow(blurred_noisy)  # 显示添加噪声且运动模糊的图像
